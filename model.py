...
class cLN(nn.Module):
    def __init__(self, dimension, eps = 1e-5, trainable=True):
    ...
    def forward(self, input):
        ...
        cumulative_mean = cumulative_sum / entry_cnt  
        cumulative_var = (cum_pow_sum - 2*cumulative_mean*cumulative_sum) / entry_cnt + cumulative_mean.pow(2)  
        cumulative_std = (cumulative_var + self.eps).sqrt()  
        
        cumulative_mean = cumulative_mean.unsqueeze(1)
        cumulative_std = cumulative_std.unsqueeze(1)
        
        a = (input - cumulative_mean.expand_as(input)) / cumulative_std.expand_as(input)
        return a * self.gain.expand_as(a).type(a.type()) + self.bias.expand_as(a).type(a.type())
    
def repackage_hidden(hid):
    if type(hid) == Variable:
        return Variable(hid.data)
    else:
        return tuple(repackage_hidden(z) for z in hid)
...
class FCLayer(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True, nonlinearity=None):
        ...
        if nonlinearity:
            self.nonlinearity = getattr(Func, nonlinearity)
        else:
