...
def calc_sdr(estimation, origin, mask=None):
    ...
    origin_power = np.sum(origin**2, 1, keepdims=True) + 1e-5  
    scale = np.sum(origin*estimation, 1, keepdims=True) / origin_power  
...
def calc_sdr_torch(estimation, origin, mask=None):
    ...
    origin_power = torch.pow(origin, 2).sum(1, keepdim=True) + 1e-5
